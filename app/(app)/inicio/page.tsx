...
export default function InicioPage() {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1100, margin: '0 auto' }}>
      ...
      {/* Las 8 PASOS oficiales del proceso Feeling */}
      <div style={{
        ...
        <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 16 }}>
          8 pasos desde la solicitud hasta el pago
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
          {[
            { num: 1, label: 'Solicitud',       actor: 'Solicitante',      path: '/solicitudes/nueva', color: '#DBEAFE', colorText: '#1E40AF' },
            { num: 2, label: 'Cotización',      actor: 'Encargado',        path: '/cotizaciones',      color: '#DBEAFE', colorText: '#1E40AF' },
            { num: 3, label: 'Crear OF',        actor: 'Encargado',        path: '/nueva-of',          color: '#E0E7FF', colorText: '#3730A3' },
            { num: 4, label: 'Aprobaciones',    actor: 'Según monto',      path: '/aprobaciones',      color: '#E0E7FF', colorText: '#3730A3' },
            { num: 5, label: 'Radicación',      actor: 'Auxiliar Compras', path: '/radicacion',        color: '#FEF3C7', colorText: '#92400E' },
            { num: 6, label: 'Auditoría',       actor: 'Auditor',          path: '/auditoria',         color: '#FEF3C7', colorText: '#92400E' },
            { num: 7, label: 'Pago',            actor: 'Tesorería',        path: '/pagos',             color: '#F3F4F6', colorText: '#374151' },
            { num: 8, label: 'Cierre',          actor: 'Contraloría',      path: '/contraloria',       color: '#F3F4F6', colorText: '#374151' },
          ].map(paso => {
            const puedeAcceder = ['admin_compras', 'gerencia'].includes(usuario?.rol || '') ||
              (usuario?.rol === 'encargado' && paso.num <= 4) ||
              (usuario?.rol === 'solicitante' && paso.num === 1)

            return (
              <div
                key={paso.num}
                onClick={() => puedeAcceder && router.push(paso.path)}
                style={{
                  background: paso.color, padding: 12, borderRadius: 6,
                  cursor: puedeAcceder ? 'pointer' : 'default',
                  opacity: puedeAcceder ? 1 : 0.5,
                  transition: 'transform 0.15s'
                }}
                onMouseEnter={e => {
                  if (puedeAcceder) e.currentTarget.style.transform = 'translateY(-2px)'
                }}
                onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}
              >
                <div style={{
                  fontSize: 10, fontWeight: 700, color: paso.colorText,
